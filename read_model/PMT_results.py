...
import uproot
from bc_utils.pmtutils import pic,plotters
...
for tpc in range(2):
  ...
  #No photon reflections or dispersion
  fig,ax = plotters.plot_TPC(tpc,'ophit_pred_nodisp_noref','$N_{pred}\ d(x) = 1$ (No reflections)',pmt_hits_df_trunc)
  plotters.save_plot(f'ophit_pred_TPC{tpc}_nodisp_noref')
  plt.close()
  fig,ax = plotters.plot_TPC(tpc,'pred_err_nodisp_noref','$ (N_{pred}-N_{obs})/N_{obs} \ d(x) = 1$ (No reflections)',pmt_hits_df_trunc)
  plotters.save_plot(f'pred_err_TPC{tpc}_nodisp_noref')
  plt.close()
